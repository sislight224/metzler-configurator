...
const TextleistePanelForm = observer(() => {
  const { setActivePanelId, updateConfig, configId, getPanelsConfig } = usePanelsStore();
  const {
    state,
    isCompleted,
    setIsCompleted,
  ...
  const beschriftungChangeHandler = useCallback((e: ChangeEvent<HTMLInputElement>) => {
  ...
  const textausrichtungChangeHandler = useCallback((_: ChangeEvent<HTMLInputElement>, value: string) => {
    addStateRecord();
  ...
  const completeClickHandler = useCallback(() => {
    ...
    getPanelsConfig(configId)
      .then((response) => {
        updateConfig<TextleistePanelState>(configId, PanelId.TEXTLEISTE, state, response.payload);
      });
    setActivePanelId(null);

    if (!isCompleted) setIsCompleted(true);
  }, [isCompleted, state, configId, beschriftung, beschriftungOberhalbIsEnabled]);

  return (
    <div className={styles.root}>
      <div className={styles.switcher}>
        <InfoSection hintText="Eindrucksvolle 3D Beschriftungsleiste aus Edelstahl überhalb der Briefkastenanlage.
        ...
        <>
          ...
          <div className={styles.section}>
            ...
            >
              <Radio
                title="Mitte"
                value={Textausrichtung.MITTE}
              />
              <Radio
                title="Links"
                value={Textausrichtung.LINKS}
              />
              <Radio
                title="Rechts"
                value={Textausrichtung.RECHTS}
              />
            </RadioGroup>
          </div>
          <Switch
