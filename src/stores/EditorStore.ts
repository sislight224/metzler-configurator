...
export default class EditorStore {
  ...
  private montagePanelSubscribe(VARS: typeof window._VARS): void {
    this.panelsStore.montagePanelStore.state.subscribe(
      "setMontageType",
      (type) => {
  ...
  private zusatzmodulPanelSubscribe(VARS: typeof window._VARS): void {
    ...
    this.panelsStore.zusatzmodulPanelStore.state.subscribe(
      ...
      (value) => {
        ...
        this.puzzles.procedures.cofigure_skeleton();
        VARS._STATE_skeleton.cam_addon_target =
          this.panelsStore.zusatzmodulPanelStore.state.zusatzmodulType.value !==
          "ohne";
        this.puzzles.procedures.cam_position();
  ...
  private textleistePanelSubscribe(VARS: typeof window._VARS): void {
    this.panelsStore.textleistePanelStore.state.subscribe(
    ...
    this.panelsStore.textleistePanelStore.state.subscribe(
      "setSide",
      (value) => {
        if (value === Textausrichtung.MITTE)
          VARS._STATE_top_lable.top_text = "center";
        if (value === Textausrichtung.RECHTS)
          VARS._STATE_top_lable.top_text = "right";
        if (value === Textausrichtung.LINKS)
          VARS._STATE_top_lable.top_text = "left";
        this.puzzles.procedures.cofigure_skeleton();
        this.setNames();
      }
    );
  ...
  private klingetableuPanelSubscribe(VARS: typeof window._VARS): void {
    this.panelsStore.klingetableuPanelStore.state.subscribe(
      "setKlingColor",
      (value) => {
  ...
  private briefkastenPanelSubscribe(VARS: typeof window._VARS): void {
    this.panelsStore.briefkastenPanelStore.state.subscribe(
      "setVisibleDies",
      (value) => {
  ...
  private zusatzmodulErweiterunPanelSubscribe(VARS: typeof window._VARS): void {
    this.panelsStore.zusatzmodulErweiterunPanelStore.state.subscribe(
      "setRfid",
      (value) => {
  ...
  private lichttasterPanelSubscribe(VARS: typeof window._VARS): void {
    this.panelsStore.lichttasterPanelStore.state.subscribe(
      "setLight",
      (value) => {
        VARS._STATE_addon_module.light_button = value;
        this.puzzles.procedures.configure_addon_module();
  ...
  private setNames(): void {
    setTimeout(() => {
      const VARS = window._VARS;
      VARS._STATE_addon_plates.names =
        this.panelsStore.klingetableuPanelStore.state.namensschildList.map(
          (item) => item.value
        );

      VARS._STATE_basic_plates.names =
        this.panelsStore.briefkastenPanelStore.state.namensschildList.map(
          (item) => item.value
        );
